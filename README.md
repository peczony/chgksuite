...
Также для всех режимов работает опция `--debug` для вывода отладочной информации. Если у вас что-то не работает или работает не так, как нужно, [создайте тикет](https://bitbucket.org/pecheny/chgksuite/issues?status=new&status=open) и пришлите туда файлы `composer.log`, `parser.log` (лежат в папке с _chgksuite_) и `debug*.json` (лежат в папке с `docx` или `4s` файлами).
