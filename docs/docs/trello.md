...
Некоторые важные галочки:

- **Формат Своей игры**: полезно, если вы редактируете в трелло не КВРМ, а свояк. Отличается от режима КВРМ тем, что в режиме КВРМ заголовок карточки игнорируется, а в режиме свояка заголовок карточки — это название темы.
  - **Без ответов**, **Только ответы** — тоже галочки для свояка. Первая полезна для текстового тестирования, а вторая для проверки письменного отбора.
- **Убрать двойные переносы строк**: chgksuite поддерживает только одинарные переносы строк внутри вопроса, но вручную следить за этим неудобно. Вы можете поставить эту галочку, и тогда chgksuite при выгрузке из Трелло уберёт лишние переносы строк за вас.
